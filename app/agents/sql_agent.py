...
from typing import Dict, List, Opional, Any, TypedDict, Annotated
import os
...
# create a our components instances
...
def parse_schema(state: AgentState) -> AgentState:
    ...
    try:
        # parse the schema
        schema_info = schema_parser.parse_schema(question)
...
def generate_query(state: AgentState) -> AgentState:
    ...
    if not isinstance(last_message, HumanMessage):
        ...
        quesion = last_user_message.content
    ...
    try:
        ...
        generated_query = query_generator.generate_query(
            ...
            dialect_features=db_handler.get_dialect_specific_features() if db_handler.dialect_name else None
        )
...
def validate_query(state: AgentState) -> AgentState:
    ...
    try:
        # Validate the query
        validation_result = query_validator.validate(
        ...
        if validation_result["is_valid"]:
            ...
            if validation_result["syntax_error"]:
                issues.append(validation_result["syntax_error"])
            issues.extend(validation_result["common_issues"])
            issues.extend(validation_result["schema_issues"])
            issues.extend(validation_result["llm_validation"]["issues"])
            
            state.messages.append(
                AIMessage(content=f"I found some issues with the SQL query:\n\n" + "\n".join([f"- {issue}" for issue in issues]) + 
                          f"\n\nI'll correct these issues and generate a new query:\n\n```sql\n{validation_result['corrected_query']}\n```")
            )
...
def execute_query(state: AgentState) -> AgentState:
    ...
    try:
        # Execute the query
        success, result = db_handler.execute_query(state.generated_query["query"])
...
"""
        ...
        response = llm.run_chain(prompt_template=prompt, output_parser=LLMResponseSchemas.common_output_parser)
